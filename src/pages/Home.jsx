import { Link } from "react-router-dom";
import Property from "./Property";
import { useState } from "react";
import properties from "../Utils/properties";
const Home = () => {
  const [search, setSearch] = useState([])
  const [propertyType, setPropertyType] = useState([])
  const [bhk, setBHK] = useState([])
  const [bhkVal, setbhkVal] = useState("")
  const [propertyTypeVal, setPropertyTypeVal] = useState("")
  const price = properties.map((value) => value.price_inr);
  const [minPrice, setMinPrice] = useState("");
  const [maxPrice, setMaxPrice] = useState("");
  const filterPrices = properties.filter((item) => {
    return (
      (minPrice === "" || item.price_inr >= minPrice) &&
      (maxPrice === "" || item.price_inr <= maxPrice)
    );
  });
console.log(properties)
  const [SearchValue, setSearchValue] = useState("");
  const [SearchCategory, setSearchCategory] = useState("");
  const [SearchBhkl, setSearchBhk] = useState("");

  const handleFilter = () => {
    const filtered = properties.filter((item) => {
      const matchesTitle = item.property_title.toLowerCase().includes(SearchValue.toLowerCase());

      const matchesCategory = SearchCategory === "" || item.property_type.toLowerCase() === SearchCategory.toLowerCase();

      const matchesBhk = SearchBhkl === "" || item.bhk.toString() === SearchBhkl;

      return matchesTitle && matchesCategory && matchesBhk;
    });

    setSearch(filtered);
  };


  return (

    <>
      <section
        className="relative h-[90vh] flex items-center justify-center bg-cover bg-center"
        style={{
          backgroundImage:
            "url('https://images.unsplash.com/photo-1505691938895-1758d7feb511')",
        }}
      >
        {/* Overlay */}
        <div className="absolute inset-0 bg-black/50"></div>

        <div className="relative max-w-6xl mx-auto px-6 text-white w-full">

          {/* Heading */}
          <h1 className="text-5xl font-bold leading-tight">
            We will help you find <br />
            your <span className="text-green-500">Wonderful</span> home
          </h1>

          <p className="mt-4 text-gray-200 max-w-xl">
            A great platform to buy, sell and rent your properties without any
            agent or commissions.
          </p>

          {/* Search Box */}
          <div className="mt-10 bg-white/30 backdrop-invert backdrop-opacity-19 rounded-xl shadow-xl p-6 text-black">

            {/* Buy Sell Rent */}
            <ul className="flex gap-4 mb-6">
              <li>
                <Link
                  to="/buy"
                  className="bg-green-600 text-white px-6 py-2 rounded-md"
                >
                  Buy
                </Link>
              </li>

              <li>
                <Link
                  to="/sell"
                  className="bg-green-600 text-white px-6 py-2 rounded-md"
                >
                  Sell
                </Link>
              </li>

              <li>
                <Link
                  to="/rent"
                  className="bg-green-600 text-white px-6 py-2 rounded-md"
                >
                  Rent
                </Link>
              </li>
            </ul>

            {/* Search Fields */}
            <div className="grid md:grid-cols-4 gap-4">

              <div>
                <label className="text-sm font-medium">Search :</label>
                <input
                  onChange={(e) => { setSearchValue(e.target.value.toLowerCase()) }}
                  type="text"
                  placeholder="Search your keywords"
                  className="w-full mt-2 border rounded-lg p-2"
                />
              </div>

              <div>
                <label className="text-sm font-medium">Select Categories :</label>
                <select
                  className="w-full mt-2 border rounded-lg p-2"
                  onChange={(e) => setSearchCategory(e.target.value)}
                >
                  <option value="">-Select-</option>
                  {propertyType.map((pt, index) => (
                    <option key={index} value={pt}>
                      {pt}
                    </option>
                  ))}
                </select>
              </div>
              <div>
                <label className="text-sm font-medium">Select BHK :</label>
                <select className="w-full mt-2 border rounded-lg p-2" onChange={(e) => setSearchBhk(e.target.value)}>
                  <option value={""}>-Select-</option>
                  {bhk.map((pt, index) => (
                    <option key={index} value={pt}>{pt}</option>
                  ))}
                </select>
              </div>

              <div>
                <label className="text-sm font-medium">Min Price :</label>
                <select className="w-full mt-2 border rounded-lg p-2" value={minPrice} onChange={(e) => setMinPrice(e.target.value)}>
                  <option value={""}>-Select-</option>

                </select>
              </div>

              <div>
                <label className="text-sm font-medium">Max Price :</label>
                <select className="w-full mt-2 border rounded-lg p-2" onChange={(e) => setMaxPrice(e.target.value)}>
                  <option value={""}>-Select-</option>

                </select>
              </div>

            </div>

            <button className="mt-6 bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-lg" onClick={() => handleFilter()}>
              Search
            </button>

          </div>
        </div>
      </section>
      <section class="py-16">

        <div class="max-w-7xl mx-auto px-6">
          <div class="text-center mb-12">
            <h2 class="text-3xl font-semibold text-gray-800">
              Featured Properties
            </h2>

            <p class="text-gray-500 mt-3 max-w-xl mx-auto">
              A great platform to buy, sell and rent your properties without any agent or commissions.
            </p>
          </div>
        </div>

        <div>
          <Property search={search} />
        </div>

      </section>
      <div class="w-full bg-slate-50 px-6 py-12">
        <div class="max-w-screen-xl mx-auto">
          <div class="max-w-3xl mb-12 max-md:text-center">
            <h2 class="text-slate-900 text-3xl font-bold mb-4 md:!leading-[45px] leading-[40px]">Discover Our Exclusive Features</h2>
            <p class="text-slate-600 text-[15px] leading-relaxed">Unlock a world of possibilities with our exclusive features. Explore how our unique offerings can transform your journey and empower you to achieve more.</p>
          </div>

          <div class="grid lg:grid-cols-3 md:grid-cols-2 max-md:max-w-lg mx-auto gap-8">
            <div class="bg-white border border-gray-200 p-5 rounded-2xl">
              <div class="flex items-center gap-6">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-[52px] h-[52px] bg-blue-50 fill-blue-700 p-3 rounded-lg shrink-0" viewBox="0 0 100 100">
                  <path d="M65.156 4.42c-8.327 0-15.13 6.855-15.13 15.202s6.803 15.165 15.13 15.165c7.017 0 12.924-4.863 14.626-11.382h13.843a3.798 3.798 0 0 0 3.791-3.805 3.798 3.798 0 0 0-3.79-3.8h-13.86C78.053 9.294 72.16 4.42 65.156 4.42zM6.391 15.8a3.798 3.798 0 0 0-3.79 3.805 3.798 3.798 0 0 0 3.79 3.8h36.397c-.21-1.234-.348-2.493-.348-3.783 0-1.304.134-2.575.348-3.821zm28.47 18.987c-7.018 0-12.92 4.89-14.619 11.418H6.392a3.783 3.783 0 0 0-.363 0 3.801 3.801 0 0 0-3.52 4.062 3.798 3.798 0 0 0 3.882 3.535H20.25c1.71 6.511 7.604 11.382 14.61 11.382 8.328 0 15.167-6.848 15.167-15.195 0-8.347-6.84-15.202-15.166-15.202zm22.383 11.418c.21 1.234.347 2.494.347 3.784 0 1.3-.134 2.57-.347 3.813h36.381a3.795 3.795 0 0 0 3.874-3.714 3.796 3.796 0 0 0-3.874-3.883H57.244zm7.912 18.979c-8.327 0-15.13 6.855-15.13 15.202S56.83 95.58 65.157 95.58c7.007 0 12.907-4.87 14.618-11.382h13.851a3.796 3.796 0 0 0 3.706-3.883 3.795 3.795 0 0 0-3.706-3.714H79.782c-1.701-6.527-7.608-11.418-14.626-11.418zM6.029 76.602a3.801 3.801 0 0 0-3.52 4.062 3.798 3.798 0 0 0 3.882 3.535h36.412a22.541 22.541 0 0 1-.348-3.813c0-1.29.138-2.55.348-3.784H6.39a3.783 3.783 0 0 0-.362 0z" data-original="#000000" />
                </svg>
                <div>
                  <h3 class="text-slate-900 text-lg font-semibold">Customization</h3>
                  <p class="text-[13px] text-slate-600 mt-1">Fits your brand needs</p>
                </div>
              </div>
              <hr class="border-gray-200 my-6" />
              <div>
                <p class="text-slate-600 text-[15px] leading-relaxed">Easily tailor every aspect of the platform to suit your business needs. From design tweaks to functional preferences and operate.</p>
              </div>
            </div>

            <div class="bg-white border border-gray-200 p-5 rounded-2xl">
              <div class="flex items-center gap-6">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-[52px] h-[52px] bg-green-50 text-green-700 p-3 rounded-lg shrink-0" viewBox="0 0 682.667 682.667">
                  <defs>
                    <clipPath id="b" clipPathUnits="userSpaceOnUse">
                      <path d="M0 512h512V0H0Z" data-original="#000000" />
                    </clipPath>
                  </defs>
                  <mask id="a">
                    <rect width="100%" height="100%" fill="#fff" data-original="#ffffff" />
                  </mask>
                  <g mask="url(#a)">
                    <g fill="none" stroke="currentColor" stroke-miterlimit="10" stroke-width="30" clip-path="url(#b)" transform="matrix(1.33333 0 0 -1.33333 0 682.667)">
                      <path d="M458.172 372.633a533.882 533.882 0 0 0-.474 16.345c-.303 20.475-16.411 37.184-36.856 38.326-62.529 3.493-111.431 24.292-152.737 64.553-6.912 6.336-17.279 6.336-24.191 0-41.306-40.261-90.208-61.06-152.737-64.553-20.445-1.142-36.553-17.851-36.857-38.325a530.642 530.642 0 0 0-.473-16.346C51.549 251.97 48.104 86.598 248.803 16.615a22.014 22.014 0 0 1 2.942-.801l.01-.002a21.72 21.72 0 0 1 8.509 0c1.002.2 1.996.47 2.961.807C463.342 86.602 460.47 251.398 458.172 372.633Z" data-original="#000000" />
                      <path d="M368.408 256c0-62.082-50.327-112.409-112.408-112.409S143.592 193.918 143.592 256c0 62.082 50.327 112.409 112.408 112.409S368.408 318.082 368.408 256Z" data-original="#000000" />
                      <path stroke-linecap="round" d="m303.227 284.952-69.785-69.785M206.773 241.834l26.668-26.668" data-original="#000000" />
                    </g>
                  </g>
                </svg>
                <div>
                  <h3 class="text-slate-900 text-lg font-semibold">Security</h3>
                  <p class="text-[13px] text-slate-600 mt-1">Enterprise-level data security</p>
                </div>
              </div>
              <hr class="border-gray-200 my-6" />
              <div>
                <p class="text-slate-600 text-[15px] leading-relaxed">Your privacy is our top priority. We implement industry-standard encryption and real-time threat detection to keep your data safe.</p>
              </div>
            </div>

            <div class="bg-white border border-gray-200 p-5 rounded-2xl">
              <div class="flex items-center gap-6">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-[52px] h-[52px] bg-purple-50 fill-purple-700 p-3 rounded-lg shrink-0" fill-rule="evenodd" viewBox="0 0 24 24">
                  <g transform="matrix(1.06 0 0 1.06 -.72 -.72)">
                    <circle cx="5.25" cy="5" r=".75" data-original="#000000" />
                    <circle cx="7.75" cy="5" r=".75" data-original="#000000" />
                    <circle cx="10.25" cy="5" r=".75" data-original="#000000" />
                    <path d="M22.75 13V4.5a3.25 3.25 0 0 0-3.25-3.25h-15A3.25 3.25 0 0 0 1.25 4.5v15a3.25 3.25 0 0 0 3.25 3.25H12a.75.75 0 0 0 0-1.5H4.5a1.75 1.75 0 0 1-1.75-1.75v-15c0-.967.783-1.75 1.75-1.75h15c.967 0 1.75.783 1.75 1.75V13a.75.75 0 0 0 1.5 0z" data-original="#000000" />
                    <path d="M22 7.25H2a.75.75 0 0 0 0 1.5h20a.75.75 0 0 0 0-1.5zm-.543 14.32-2.101-3a.75.75 0 1 0-1.228.86l2.1 3a.75.75 0 0 0 1.229-.86z" data-original="#000000" />
                    <path d="m22.067 16.084-6.47-3.733a1.367 1.367 0 0 0-2.027 1.419l1.295 7.357a1.365 1.365 0 0 0 2.428.594l1.754-2.286 2.748-.866a1.366 1.366 0 0 0 .272-2.485zm-1.015 1.146-2.682.846a.742.742 0 0 0-.369.258l-1.712 2.232-1.192-6.772z" data-original="#000000" />
                  </g>
                </svg>
                <div>
                  <h3 class="text-slate-900 text-lg font-semibold">User-Friendly Interface</h3>
                  <p class="text-[13px] text-slate-600 mt-1">Streamlined for productivity</p>
                </div>
              </div>
              <hr class="border-gray-200 my-6" />
              <div>
                <p class="text-slate-600 text-[15px] leading-relaxed">Navigate with ease using our intuitive interface designed for efficiency. Whether you're a beginner or expert.</p>
              </div>
            </div>

            <div class="bg-white border border-gray-200 p-5 rounded-2xl">
              <div class="flex items-center gap-6">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-[52px] h-[52px] bg-orange-50 fill-orange-700 p-3 rounded-lg shrink-0" viewBox="0 0 512 512">
                  <path d="M451 257v215c0 22.5-14.1 40-32.1 40H375c-18 0-32.1-17.6-32.1-40V257c0-22.5 14.1-40 32.1-40h43.9c17.9 0 32.1 17.6 32.1 40zm.7-126.1c-3 2.1-6.9 2.2-10.1.3l-30-18C362.2 195 292.5 272 157.9 272c-28.4 0-59.7-3.4-94.3-11-5-1.1-8.2-6-7.2-11.1 1-4.6 5.2-7.7 9.9-7.3 8.4.7 203.6 13.8 285.6-166.7L321.2 61c-4.6-2.2-6.6-7.8-4.4-12.4 1-2.1 2.7-3.7 4.8-4.6L423.5.7c4.7-2 10.2.2 12.2 4.9.3.7.5 1.4.6 2.1l19.3 113.9c.7 3.6-.9 7.3-3.9 9.3zM310.1 336v136c0 22.5-14.1 40-32.1 40h-44c-18 0-32.1-17.6-32.1-40V336c0-22.5 14.1-40 32.1-40h43.9c18.1-.1 32.2 17.5 32.2 40zm-137.8 65.8V472c0 22.4-14.1 40-32.1 40h-44c-18 0-32.1-17.6-32.1-40v-70.2c0-22.5 14.1-40 32.1-40h43.9c18.1-.1 32.2 17.5 32.2 40z" data-original="#000000" />
                </svg>
                <div>
                  <h3 class="text-slate-900 text-lg font-semibold">Performance</h3>
                  <p class="text-[13px] text-slate-600 mt-1">Fast and dependable</p>
                </div>
              </div>
              <hr class="border-gray-200 my-6" />
              <div>
                <p class="text-slate-600 text-[15px] leading-relaxed">Experience lightning-fast load times and seamless performance. Our system is built to deliver results without lag or disruption.</p>
              </div>
            </div>

            <div class="bg-white border border-gray-200 p-5 rounded-2xl">
              <div class="flex items-center gap-6">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-[52px] h-[52px] bg-pink-50 text-pink-700 p-3 rounded-lg shrink-0" viewBox="0 0 682.667 682.667">
                  <defs>
                    <clipPath id="a" clipPathUnits="userSpaceOnUse">
                      <path d="M0 512h512V0H0Z" data-original="#000000" />
                    </clipPath>
                  </defs>
                  <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="30" clip-path="url(#a)" transform="matrix(1.33333 0 0 -1.33333 0 682.667)">
                    <path d="M75 467c0-16.568-13.432-30-30-30-16.568 0-30 13.432-30 30 0 16.568 13.432 30 30 30 16.568 0 30-13.432 30-30ZM75 45c0-16.568-13.432-30-30-30-16.568 0-30 13.432-30 30 0 16.568 13.432 30 30 30 16.568 0 30-13.432 30-30ZM497 467c0-16.568-13.432-30-30-30-16.568 0-30 13.432-30 30 0 16.568 13.432 30 30 30 16.568 0 30-13.432 30-30ZM497 45c0-16.568-13.432-30-30-30-16.568 0-30 13.432-30 30 0 16.568 13.432 30 30 30 16.568 0 30-13.432 30-30ZM406 256c0-82.843-67.157-150-150-150s-150 67.157-150 150 67.157 150 150 150 150-67.157 150-150Z" data-original="#000000" />
                    <path d="M316 256c0-82.843-26.863-150-60-150s-60 67.157-60 150 26.863 150 60 150 60-67.157 60-150ZM106 256h300M66.213 445.787l83.721-83.721M445.787 445.787l-83.721-83.721M362.066 149.934l83.721-83.721M149.934 149.934 66.213 66.213" data-original="#000000" />
                  </g>
                </svg>
                <div>
                  <h3 class="text-slate-900 text-lg font-semibold">Scalability</h3>
                  <p class="text-[13px] text-slate-600 mt-1">Scales with your growth</p>
                </div>
              </div>
              <hr class="border-gray-200 my-6" />
              <div>
                <p class="text-slate-600 text-[15px] leading-relaxed">Whether you're a startup or enterprise. Add users, features, or capabilities without disrupting performance.</p>
              </div>
            </div>

            <div class="bg-white border border-gray-200 p-5 rounded-2xl">
              <div class="flex items-center gap-6">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-[52px] h-[52px] bg-blue-50 fill-blue-700 p-3 rounded-lg shrink-0" viewBox="0 0 512 512">
                  <path d="M495.984 252.588c-17.119-14.109-44.177-15.319-61.936 3.74l-44.087 47.327c-5.7-18.319-22.809-31.658-42.977-31.658h-78.675c-5.97 0-7.969-2.28-18.339-10.269-39.538-34.468-98.924-34.358-138.342.33L82.71 287.516c-12.999-6.88-28.178-7.05-41.248-.52L8.294 303.575c-7.41 3.71-10.409 12.719-6.71 20.129l89.995 179.989c3.71 7.41 12.719 10.409 20.129 6.71l33.168-16.589c16.349-8.169 25.448-24.849 24.858-41.827h177.249c32.868 0 64.276-15.699 83.995-41.997l72.006-96.014c13.969-18.61 11.759-45.899-7-61.388zM131.456 466.985l-19.749 9.879-76.585-153.16 19.759-9.879c7.41-3.7 16.409-.71 20.119 6.71l63.166 126.332c3.7 7.409.7 16.408-6.71 20.118zm347.529-171.009L406.98 391.99c-14.089 18.789-36.518 29.998-59.996 29.998H159.265l-56.207-112.423 28.388-24.988c28.248-24.849 70.846-24.849 99.094 0 16.639 14.649 26.988 17.419 37.768 17.419h78.675c8.27 0 14.999 6.73 14.999 14.999s-6.73 14.999-14.999 14.999h-76.605c-8.28 0-14.999 6.72-14.999 14.999s6.72 14.999 14.999 14.999h86.655c12.449 0 24.449-5.22 32.928-14.329l66.036-70.886c6.04-6.48 15.299-5.94 20.979-.97 5.939 5.199 6.58 14.089 2.009 20.169zm-163.6-193.609c10.269-10.769 16.599-25.328 16.599-41.358 0-33.018-26.678-60.996-59.996-60.996-33.068 0-60.996 27.928-60.996 60.996 0 15.539 6.09 30.208 17.149 41.478-27.428 15.379-47.147 44.897-47.147 79.515v14.999c0 8.279 6.72 14.999 14.999 14.999h150.991c8.279 0 14.999-6.72 14.999-14.999v-14.999c-.001-33.938-18.668-63.916-46.598-79.635zm-43.397-72.355c16.259 0 29.998 14.199 29.998 30.998 0 16.539-13.459 29.998-29.998 29.998-16.799 0-30.998-13.739-30.998-29.998 0-16.509 14.489-30.998 30.998-30.998zm-60.996 151.99c0-33.068 27.928-60.996 60.996-60.996 33.078 0 59.996 27.358 59.996 60.996H210.992z" data-original="#000000" />
                </svg>
                <div>
                  <h3 class="text-slate-900 text-lg font-semibold">24/7 Support</h3>
                  <p class="text-[13px] text-slate-600 mt-1">Help is always within reach</p>
                </div>
              </div>
              <hr class="border-gray-200 my-6" />
              <div>
                <p class="text-slate-600 text-[15px] leading-relaxed">Our expert support team is available around the clock to assist you. we’re here to ensure your experience remains smooth.</p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Home;
